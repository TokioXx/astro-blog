...
The **HN digest** (every day) and the **portfolio post** (US trading days only) are generated automatically by a **local macOS launchd LaunchAgent** (`com.guodong.astro-blog-daily`): it runs Claude Code headlessly at **5 PM America/Los_Angeles**, regenerates `hn-daily-DATE.md` (§1) and, on trading days, `portfolio-DATE.md` (§2), builds, and pushes to `main`. The portfolio step reads the cost basis from the latest `~/Documents/Claude/Projects/Portfolio/Portfolio_Positions_*.csv` (which stays local — only %/prices/ratings/targets are published). Runner: `~/.claude/blog-daily/run.sh`; logs: `~/.claude/blog-daily/last-run.log`; schedule: `~/Library/LaunchAgents/com.guodong.astro-blog-daily.plist`. Manage with `launchctl list | grep astro-blog-daily` and `launchctl bootout/bootstrap gui/$(id -u) <plist>`. The **watchlist** post is still generated manually. The steps below are the same whether a human or the agent runs them.
